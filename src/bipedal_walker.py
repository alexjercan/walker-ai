...
FOOT_FD = fixtureDef(
    shape=polygonShape(box=(FOOT_S / 2, FOOT_S / 2)),
    density=1.0,
    restitution=0.0,
    categoryBits=0x0020,
    maskBits=0x001,
...
class ContactDetector(contactListener):
    ...
    def BeginContact(self, contact):
        if (
            ...
            or any([
                leg in [contact.fixtureA.body, contact.fixtureB.body] for leg in self.env.legs[:4]
            ])
        ):
            self.env.game_over = True
        for leg in [self.env.legs[4], self.env.legs[5]]:
...
class BipedalWalker(gym.Env, EzPickle):
        ...
        *,
        ...
        for i in [-1, 1]:
            foot = self.world.CreateDynamicBody(
                position=(init_x, init_y - LEG_H * 2 - FOOT_S / 2 - LEG_DOWN),
                angle=(i * 0.05),
                fixtures=FOOT_FD,
            )
    ...
    def step(self, action: np.ndarray):
        ...
        state = [
            ...
            self.joints[1].angle + 1.0,
            self.joints[1].speed / SPEED_KNEE,
            1.0 if self.legs[4].ground_contact else 0.0,
            self.joints[2].angle,
            self.joints[2].speed / SPEED_HIP,
            self.joints[3].angle + 1.0,
            self.joints[3].speed / SPEED_KNEE,
            1.0 if self.legs[5].ground_contact else 0.0,
        ]
        state += [l.fraction for l in self.lidar]
        assert len(state) == 24
